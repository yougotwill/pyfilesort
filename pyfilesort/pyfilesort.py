...
# Classify folders that have fullstops but aren't app containers

# add flag to sort the primary files from the command line
# i.e. clean dowinloads, desktop, etc.

# make flags work with sys.argv

# Make separate config file that will be read into the program.
# Will contain folder_dic and folder flag and other possible settings.

# Create an exceptions menu options where users can add files and folers
# that should be ignored by pyfilesore i.e. the incomplete folder in my case

# os -  used for os manipulation such as reading or writing to a file
...
# torrenttool - python library to read a torrent to find the matching directory

import os
import shutil
import sys
# library used to read torrent meta data so the folder
# can be classified along with the torrent file
...
folder_dic = {
    "_Books": [".epub", ".mobi"],
    "_Documents": [
        ".pdf", ".txt", ".doc", ".docx", ".dotx", ".ppt", ".pptx", ".md",
        ".json", ".ods", ".log", ".xls", ".xlsx"
    ],
    "_Images": [".png", ".jpg", ".jpeg", ".gif", ".xcf", ".stl", ".blend"],
    "_Music": [
        ".mp3", ".wav", ".flac", ".m4a", ".ogg", ".mid", ".asd", ".m3u",
        ".pls", ".alp", ".asx", ".bfxrsound"
    ],
    ...
    "_Web": [".html", ".css", ".js"],
    "_Zipped":
    [".zip", ".rar", ".7z", ".tar.gz", ".tar", ".gz", ".unitypackage"]
}
folder_order = {
    "_Books": 1,
    "_Documents": 2,
...
# checks for most occurences of an _folder and returns that _folder
...
def print_sort_folder_options(
):  # prints out the _sorting folders with an allocated number flag
    output = ""
...
def process_folder(sort_folder,
                   base_folder):  #sort_folder is the folder I am classifying
    global move_count
    global folder_torrent
    #torrent code
    if (len(folder_torrent) != 0 and sort_folder in str(
            folder_torrent
    )):  #if the torrent folder array isn't empty and if the directory matches with a .torrent file
        ...
        folder = most_occurences(
            folder_string
        )  #gets the most occurences of a _folder in folder_string and that is what we classify with
        if folder != "NA":
            folder_path = base_folder + "/" + folder
            # check if a sorting folder directory exists
            if os.path.exists(folder_path) == False:
...
def process_files(base_folder):
    ...
    for filename in dirlist:
        ...
        if (base_folder == clean_folder):
            if (folder_ignore.count(filename) ==
                    0):  #checks for files or folders to ignore
